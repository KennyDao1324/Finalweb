...
Ghi chú:`;
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (name.length < 2) { setError("nameErr", "Tên phải có ít nhất 2 ký tự."); ok = false; }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { setError("emailErr", "Vui lòng nhập email hợp lệ."); ok = false; }
    if (!topic) { setError("topicErr", "Vui lòng chọn một chủ đề."); ok = false; }
    if (message.length < 10) { setError("messageErr", "Tin nhắn phải có ít nhất 10 ký tự."); ok = false; }
    if (!agree) { setError("agreeErr", "Bạn cần đồng ý với điều khoản."); ok = false; }
